...
const float norm_factor = { 1000.0 };
...
bool	Distort = 0;		// global -- true means to distort the texture
...
MjbSphere(float radius, int slices, int stacks)
{
	...
	for (int ilat = 0; ilat < NumLats; ilat++)
	{
		...
		for (int ilng = 0; ilng < NumLngs; ilng++)
		{
			...
			p->ny = y;
			p->nz = z;
			//if (Distort && (WhichDistort == DISTORT_ANIMATION))
			//{
			//	p->s = (lng + lat*angle_factor / 100 + M_PI) / (2.*M_PI);
			//	//p->s = (lng +lat*tan(angle_factor*M_PI / 360 )+ M_PI) / (2.*M_PI);
			//	p->t = (lat + M_PI / 2.) / M_PI;
			//}
			//else if (Distort && (WhichDistort == DISTORT))
			//{
			//	p->s = (lng + lat + M_PI) / (2.*M_PI);
			//	p->t = (lat + M_PI / 2.) / M_PI;
			//}
			//else
			//{
...
Display()
{
	...
	//glEnable(GL_LIGHT0);
	//glLightfv(GL_LIGHT0, GL_POSITION, Array3(5.0, 5.0, 0));
	...
	S0= 0.5;
	T0 = 0.5;
	//Size = .1*(1.5 + 1.5*sin(2.*M_PI*Time));
	// draw the current object:
	Pattern->Use();
	Pattern->SetUniformVariable("fragment", showFrag);
	Pattern->SetUniformVariable("vertex", showVert);
	...
	glLoadIdentity();
	glColor3f(1., 1., 1.);
	//DoRasterString(5., 5., 0., "Designed by Yongbin Gu");
